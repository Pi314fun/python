...
def summary(cur) :
    ...
    good = myutils.queryValue(cur, 'SELECT COUNT(*) FROM pokeapi WHERE status = 200;')
    error = myutils.queryValue(cur, 'SELECT COUNT(*) FROM pokeapi WHERE status != 200;')
...
if count < 1:
    objects = ['1', '2', '3']
    for obj in objects:
        sql = f"INSERT INTO swapi (url) VALUES ( 'https://pokeapi.co/api/v2/pokemon/{obj}/' )";
        print(sql)
...
while True:
    ...
    links = ['id', 'name', 'version-group', 'pokemon', 'generation']
    for link in links:
